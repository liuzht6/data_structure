...
void initialize(int& end_time, int& capacity, int& queue_limit,
	double& arrival_rate, double& departure_rate, double& fuel_rate) {
	...
	do {
		...
		cin >> departure_rate;
		cout << "Expected number of fuel per plane? " << flush;
		cin >> fuel_rate;
		if (arrival_rate < 0.0 || departure_rate < 0.0 || fuel_rate < 0.0)
