...
class Airport {
 public:
  ...
  bool can_land(const Plane& current) {
    num_land_requests++;
    //change all runway to land
    if (landing_queue.size() == queue_limit && !takeoff_queue.empty() && alltoland == false) {
    ...
    else if (size < capacity && landing_queue.size() < queue_limit * 2 && takeoff_queue.size() == 0) {
      landing_queue.push(current);
      size++;
      num_land_accepted++;
      return true;
  ...
  bool can_depart(const Plane& current) {
    ...
    //change to aonther use
    else if (takeoff_queue.size() < queue_limit * 2 && landing_queue.size() == 0) {
      num_takeoff_accepted++;
      takeoff_queue.push(current);
      size--;
      return true;
  ...
  void activity(int time, Plane& moving) {
   for (auto r : Runways) {
      ...
      } else if (!landing_queue.empty() && takeoff_queue.empty() &&
                 r.state() == takeoff) {
        moving = landing_queue.front();
        r.land(time, landing_queue.front());
        landing_queue.pop();
   ...
   //change to before
   if (landing_queue.empty() && alltoland) {
       for (int i = 0; i < Runways.size(); i++)
           Runways[i].changeway();
       alltoland = false;
   }
